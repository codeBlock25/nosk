...
import 'package:nosk/src/screens/rooms/create.dart';
...
Future<File> fileFromImageUrl({required String url}) async {
  ...
  final String fileName = appDir.path + imageName;
...
class EditRoomPage extends RouteFulWidget {
  const EditRoomPage({super.key});

  static EditRoomPage get route => const EditRoomPage();

  @override
  State<EditRoomPage> createState() => _EditRoomPageState();

  @override
  String path() => '/edit-room';
...
class _EditRoomPageState extends State<EditRoomPage> {
  ...
  void initState() {
    ...
    _roomLogic.getRoom(roomName).listen((event) async {
      if (event.data() == null) return;
      final RoomModel room = event.data()!;
      _roomNameController.text = room.name;
      _descriptionController.text = room.description;
      _priceController.text = room.price.toStringAsFixed(2);
      _amenities.addAll(room.amenities);
      _roomNumbers.addAll(room.roomNumber);
      _selectedFiles = await Future.wait(
        room.images.map(
          (e) => fileFromImageUrl(url: e),
        ),
      );
      setState(() {});
    });
              ...
              children: [
                    ...
                    suffixIcon: TextButton(
                      onPressed: () {
                        ...
                        if (_roomNumbers
                            ...
                            .contains(_roomNumberController.text
                                .toLowerCase()
                                .trim())) {
                          context.toast(
                ...
                Obx(
                  () => ElevatedButton.icon(
                    ...
                    style:
                        ButtonStyle(minimumSize: Size(240, 26.cl(30, 55)).all),
